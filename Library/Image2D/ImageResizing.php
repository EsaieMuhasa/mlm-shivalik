...
class ImageResizing extends ImageBuilder
{
    ...
    public function __construct(Image $image, array $images)
    {
        ...
        if (!$this->image->fileExists()) {
            throw new LibException('Le redimensionnement d\'une image ce base sur une image existant physiquement sur le disque dur. " '.$image->getFileName().'" ');
        }
        
        if ($images === null || empty($images)) {
            throw new LibException('La collections de la description des informations de redimensionnement ne doit pas etre null ou vide.');
        }
    ...
    public function build() : void
    {
        foreach ($this->images as $img) {
            ...
            if (!$statut) {
                throw new LibException('Eche de redimensionnement de l\'image.');
            }
            ...
            if (!$statut) {
                throw new LibException('Eche d\'écriture de l\'image rédimensionner sur le disque dur');
            }
        }
    }
    ...
    public static function builder(Image $image, $remove = true) : void{
        ...
        if (!$image->fileExists()) {
            throw new LibException("Impossible de demanrer les algorithmes des redimensionnement des images. l\'image doit exister physiquement sur le serveur. > {$image->getFileName()} <");
        }
        ...
        if (preg_match('#^(.+)-reel\.(png|PNG|jpg|jpeg|JPG|JPEG)$#', $image->getFileName(), $matches)) {
            ...
            throw new LibException("impossible de poursuivre les traitements car l'outils de redimensionnement n'arrive pas a separer le nom de l'image a son extension. Image non prise en charge {$image->getFileName()}");
        }
    ...
    public static function profiling(Image $image, ?ImageRect $imageRect = null, $remove=true) : void{
        ...
        if (!$image->fileExists()) {
            throw new LibException("Impossible de demanrer les algorithmes de generation des images. l\'image doit exister physiquement sur le serveur. > {$image->getFileName()} <");
        }
        ...
        if (preg_match('#^(.+)-reel\.(png|PNG|jpg|jpeg|JPG|JPEG)$#', $image->getFileName(), $matches)) {
            ...
            throw new LibException('Les images pise en charge sont uniquement de PNG et JPG');
        }
    ...
    protected static function equalize(Image $img_source, Image $img_destination) : void{
        ...
        if (!$statut) {
            throw new LibException('Echec d\'execution de l\'operation. une erreur est survenue lors de construction de l\'image.');
        }
        ...
        if (!$statut) {
            throw new LibException('Echec d\'execution de l\'operation. une erreur est survenue lors de l\'ecriture de l\'image sur le disque dur du serveur.');
        }
    }
