...
class Month
{
    ...
    public function __construct(?int $month=null, ?int $year=null, ?int $firstDayOfWeek = null, ?string $local = null){
        ...
        if ($month<1 || $month > 12 ) {
            throw new LibException('=> '.$month.' <= Le mois doit etre une valeur compise entre 1 et 12');
        }
        
        if ($year < 1970){
            throw new LibException('=> '.$month.' <= L\'année doit être une valeur supérieur à 19970');
        }
    ...
    public function nextWeek() : int{
        ...
        throw new LibException('Impossible de de faire avancer le curseur sur la semaine suivante');
    }
