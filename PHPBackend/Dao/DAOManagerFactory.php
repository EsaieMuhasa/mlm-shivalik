...
class DAOManagerFactory
{
    ...
    public function getManagerOf($name) : DAOInterface
    {
        if (array_key_exists($name, $this->interfaces)) {
            ...
            $fileClass = dirname(dirname(__DIR__)).str_replace('\\', DIRECTORY_SEPARATOR, $meta->getImplementation()).'.php';
            
            if (!file_exists($fileClass)) {
                throw new PHPBackendException("Le fichier '{$fileClass}' de definition de l'implementation du manager de l'entite {$meta->getName()} n\'existe pas. '{$meta->getImplementation()}'");
            }
