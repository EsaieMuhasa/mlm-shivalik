...
class Image
{
     ...
     * @throws PHPBackendException si l'image n'est pas prise en charge
     */
    public function __construct(string $fileName, ?ImageRect $rectangle = null)
    {
        ...
        if ($rectangle === null) {//Si le rectagle de l'image n'a pas ete definie
            if($this->fileExists()){//si l'image existe physiquement sur le serveur
                ...
                if ($image === false) {
                    throw new PHPBackendException('Echec de chargement de la ressource '.$fileName);
                }
    ...
    protected function setFileName(string $fileName) : void{
        ...
        throw new PHPBackendException('Image non prise en charge. L\'image doit etre un PNG soit un JPG.');
    }
     ...
     * @throws PHPBackendException si l'image d'existe pas physiquement sur le serveur
     * @return ImageRect
     */
    public function getReelSize() : ImageRect{
        if (!$this->fileExists()) {
            throw new PHPBackendException('Impossible de deduire le rectagle de l\'image car il n\'existe pas physiquement sur le serveur.');
        }
