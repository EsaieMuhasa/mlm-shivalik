...
class SearchMember {
    private form : HTMLFormElement;//Le formulaire de recherche
    private container: HTMLElement;//le conteneur des resultats de recherche
    private field : HTMLInputElement;//le champ de saisie
    ...
    constructor (form : HTMLFormElement, container : HTMLElement) {
        ...
        this.field = form.querySelector('input') as HTMLInputElement;
     ...
     */
    public sendRequest ( action : string, type : string = 'POST' ) : void {
        $.ajax({
            url : action,
            type: type,
            ...
            success : (data) => {
                ...
                if(members.length != 0) {
                    ...
                    for (let i = 0; i < members.length; i++) {
                        ...
                        let desc : string = " -> Subcribe date "+member.dateAjout;
...
$(() => {
    const form = document.querySelector<HTMLFormElement>("#form-search-members");
    const container = document.querySelector<HTMLElement>('#container-search-members');
