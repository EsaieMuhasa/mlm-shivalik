 ...
 */
class PHPBackendException extends \RuntimeException
{
     ...
     * @return PHPBackendException
     * @throws PHPBackendException
     */
    public static function parseXML(string $textXML) : PHPBackendException {
        $xml = new \DOMDocument();
        $readable = $xml->loadXML($textXML);
        
        if ($readable===false){
            throw new PHPBackendException('Impossible de paser le contenue XML: '.$textXML);
        }
        
        $root = $xml->getElementsByTagName('exception');
        if ($root==null || empty($root)){
            throw new PHPBackendException('Les donnees XML en parametre sont invalide: '.$textXML);
        }
        ...
        return new PHPBackendException($message, $code);
    ...
    public function toHTML(){
        ...
        if ($this->getPrevious()!=null) {
            ...
            $html .= PHPBackendException::previousToHTML($this->getPrevious());
            $html .= '</li>';
        }
        $html .= '</ul>';
        return $html;
    ...
    protected static function previousToJSON(\Exception $previous){
        ...
        if ($previous->getPrevious()!=null) {
            $json .= ','.PHPBackendException::previousToJSON($previous->getPrevious());
        }
        $json .= '}';
        return  $json;
    }
    ...
    protected static function previousToXML(\Exception $previous){
        ...
        if ($previous->getPrevious()!=null) {
            $xml .= '<property name="previous">';
            $xml .= PHPBackendException::previousToXML($previous->getPrevious());
            $xml .= '</property>';
        }
        $xml .= '</exception>';
        return  $xml;
    ...
    protected static function previousToHTML(\Exception $previous){
        ...
        if ($previous->getPrevious()!=null) {
            $html .= '<li> Exception Predecesseuer';
            $html .= PHPBackendException::previousToHTML($previous->getPrevious());
            $html .= '</li>';
        }
        $html .= '</ul>';
        return $html;
