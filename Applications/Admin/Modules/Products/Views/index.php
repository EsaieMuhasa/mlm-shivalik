...
$offices = $_REQUEST[ProductsController::ATT_OFFICES];
	...
	<div class="col-lg-6 col-md-7 col-sm-12 col-xs-12">
		<div class="panel panel-default">
	        <header class="panel-heading">
	            ...
	            <span class="pull-right">
	            	<span class="btn-group">
    	            	<a title="<?php echo ($prev);?>" class="btn btn-primary" href="<?php echo ("/admin/products/commands-of-{$prev->getFirstDay()->format('m-Y')}");?>/">
    	        			<span class="glyphicon glyphicon-step-backward"></span>
        	        	</a>
        	        	<a title="<?php echo ($next);?>" class="btn btn-info" href="<?php echo ("/admin/products/commands-of-{$next->getFirstDay()->format('m-Y')}");?>/">
	            ...
	            <tbody>
	                ...
	                <tr>
	                    ...
	                    <td class="<?php echo (($month->isSelectedWeek($month->getCurrentWeek()))? 'danger':'');?>">
	                    	...
	                    	<a href="<?php echo ("/admin/products/commands-of-{$date->format('d-m-Y')}");?>/" class="btn btn-xs <?php echo ($month->isSelectedDate($date)? 'btn-primary ':'').($month->isToday($date)? ' text-primary':'').($month->isEventDate($date)? 'text-danger':'');?>">
	                    ...
	                    <td class="<?php echo ($month->isSelectedWeek($month->getCurrentWeek())? 'danger':'info');?>">
	                    	<a href="<?php echo ("/admin/products/commands-of-{$month->getFirstDayOfCurrentWeek()->format('d-m-Y')}-to-{$month->getLastDayOfCurrentWeek()->format('d-m-Y')}-week-{$month->getCurrentWeek()}");?>/" class="btn btn-xs">
	        ...
	        <div class="panel-footer">
	        	<div class="row">
	        		<div class="col-xs-3">
        	        	<a title="<?php echo ($month->getMonth()<10? '0'.$month->getMonth():'').' '.($month->getYear()-1);?>" class="btn btn-primary btn-block" href="<?php echo "/admin/products/commands-of-".($month->getMonth()<10? '0':'').$month->getMonth().'-'.($month->getYear()-1);?>/">
        	        	...
        	        	<div class="dropup">
                        	...
                        	<ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                        		...
                        		<li class="<?php echo ($i == $month->getMonth()? "active" : ""); ?>">
                        			<a href="<?php echo "/admin/products/commands-of-{$m->getMonth()}-{$year}";?>/"><?php echo htmlspecialchars($m->getName())?></a>
                        		</li>
                        		<?php endfor;?>
                        	</ul>
                        </div>
...
<?php foreach ($offices as $office) : 
    $key = "office-{$office->getId()}";
    if (!array_key_exists($key, $commands)) {
        continue;
    }
    $byOffice = $commands[$key]; 
?>
<div class="panel panel-default">
	<div class="panel-heading">
		<strong class="panel-title"><?php echo $office->getName(); ?></strong>
	</div>
	<div class="panel-body">
        ...
        <?php foreach ($byOffice as $command) : ?>
        <div class="thumbnail">
        	<div class="row">
        	...
        	<div class="row">
        		<div class="col-xs-12 text-right">
        			<div class=" btn-group">
        				<a class="btn btn-info" target="_black" href="<?php echo "/admin/products/commands/{$command->id}/pdf.pdf"; ?>">
